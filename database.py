...
def init_db():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany("INSERT INTO quests (name, description, target_clicks, reward_clicks) VALUES (?, ?, ?, ?)", default_quests)
        conn.commit()
...
def get_user_stats(user_id: int) -> Tuple:
    ...
    if not result:
        cursor.execute("INSERT INTO users (user_id, clicks, level, energy, tap_power, passive_income, premium_until, current_skin, total_clicks, daily_streak) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (user_id, 0, 1, 1000, 1, 0, None, '🦆', 0, 0))
        conn.commit()
...
def update_tap(user_id: int, energy_cost: int = 1, tap_bonus: int = 0) -> Tuple:
    ...
    cursor.execute("UPDATE users SET clicks = ?, level = ?, energy = ?, total_clicks = ? WHERE user_id = ?", (new_clicks, new_level, new_energy, new_total_clicks, user_id))
    conn.commit()
    conn.close()
