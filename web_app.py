...
def get_user_stats(user_id: int):
    ...
    if result:
        return {
            ...
            "tap_power": result[3], "total_clicks": result[4]
        }
    else:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO users (user_id, clicks, level, energy, tap_power, total_clicks) VALUES (?, ?, ?, ?, ?, ?)",
                       (user_id, 0, 1, 1000, 1, 0))
        conn.commit()
        conn.close()
        return {"clicks": 0, "level": 1, "energy": 1000, "tap_power": 1, "total_clicks": 0}
