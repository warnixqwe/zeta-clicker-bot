...
async def get_skins(user_id: int):
    ...
    for skin in all_skins:
        result.append({
            "id": skin[0], "name": skin[1], "emoji": skin[2], "price_clicks": skin[3],
            "price_gems": skin[4], "bonus": skin[5], "limited": skin[6],
            "owned": skin[0] in owned, "equipped": skin[2] == current
        })
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; user-select: none; -webkit-tap-highlight-color: transparent; }}
        ...
        .action-btn {{ background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; border-radius: 16px; padding: 14px; color: white; font-size: 14px; font-weight: 600; cursor: pointer; text-align: center; }}
        .action-btn:active {{ transform: scale(0.96); }}
        .back-btn {{ background: rgba(255,255,255,0.1); margin-top: 20px; }}
        .full-width {{ width: 100%; }}
        .skin-list, .booster-list, .achievement-list, .case-list, .leaderboard-list {{ margin: 20px 0; }}
        .skin-item, .booster-item, .achievement-item, .leaderboard-item {{ background: rgba(0,0,0,0.3); border-radius: 16px; padding: 12px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 10px; }}
